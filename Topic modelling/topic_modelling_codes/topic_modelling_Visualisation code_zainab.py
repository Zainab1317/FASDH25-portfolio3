...
# Filter rows where "palestine" or "israel" appear in the Topic_Label, used chatgpt to understand (entry 5 in AI doc)
filtered_df = df.copy()
filtered_df["palestine_mention"] = filtered_df["Topic_Label"].str.contains("palestine", case=False, na=False)
filtered_df["israel_mention"] = filtered_df["Topic_Label"].str.contains("israel", case=False, na=False)# case=False is written to make the check case-insensitive

# Extract month for grouping
filtered_df["month"] = filtered_df["date"].dt.to_period("M").astype(str)

# Group by month and sum the mentions #used chatgpt prompt to understand (entry 6 in AI doc)
mentions_per_month = filtered_df.groupby("month")[["palestine_mention", "israel_mention"]].sum().reset_index()

# Plot the line chart for palestine mentions
fig = px.line(
    mentions_per_month,
    x="month",
    y="palestine_mention",
    title="Monthly Mentions of 'Palestine' and 'Israel' in Topic Labels",
    labels={"value": "Mentions", "month": "Month"},
    markers=True # Adds dots to the line for clarity at each data point.
)

#have added a second line to the existing chart for Israel mentions.
fig.add_scatter(
    x=mentions_per_month["month"],
    y=mentions_per_month["israel_mention"],
    mode="lines+markers", # Using lines+markers so both the trend line and dots at each point are visible
    name="Israel Mentions"
)
# Updating the axis titles for clarity
fig.update_layout(
    yaxis_title="Number of Articles",
    xaxis_title="Month"
)
fig.show()
fig.write_html("monthly_mentionsof_palestine_or_israel_in_topiclabels_zainab.html")
